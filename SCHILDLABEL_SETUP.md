# schildlabel.com — cold-email sending domain + redirect to schildinc.com

`schildlabel.com` is your dedicated **cold-outreach** domain. Sending cold email from it (instead of
...
## Part A — Make schildlabel.com send via Resend

1. **Resend** → https://resend.com/domains → **Add Domain** → `schildlabel.com`, region **EU (eu-west-1)**.
...
3. **Fix the root SPF.** The domain currently has `v=spf1 -all` on the root (`@`) — that hard-fails
   mail claiming to be from `@schildlabel.com`. **Replace it** with:
   ```
...
5. Tell the app it's allowed to send — in Railway set/append:
   ```
   SEND_VERIFIED_DOMAINS=schildinc.com,schildlabel.com
   ```
6. **Set up reply handling.** The app sends from `ruben@schildlabel.com` and replies go there. Since
   schildlabel.com has no inbox yet, either:
   - add **email forwarding** at NameBright: `ruben@schildlabel.com` → your Trengo address / a monitored
     mailbox, **or**
   - override the reply-to in Railway: `SEND_SCHILDLABEL_COM_REPLY_TO=sales@schildinc.com`
     (any Schild-owned address is accepted; replies then land in your existing inbox).
   You can also change the From address with `SEND_SCHILDLABEL_COM_FROM` / `_NAME`.
7. Send yourself a test from the campaign screen (pick 1 recipient = your own email, "Send from" =
   schildlabel.com) and confirm it lands in the inbox, not spam.
...
## Part B — Redirect schildlabel.com → schildinc.com (website visits)
...
2. Forward `schildlabel.com` **and** `www.schildlabel.com` → `https://schildinc.com`, type **permanent (301)**.
...
1. Add `schildlabel.com` to a free Cloudflare account; it imports your DNS. **Keep the Resend `send`
...
4. **Rules → Redirect Rules → Create:** when Hostname equals `schildlabel.com` or `www.schildlabel.com`
...
The app already 301-redirects `schildlabel.com` → schildinc.com (`REDIRECT_HOSTS` includes it). To use
it, point schildlabel.com's `@`/`www` at the Railway app (Railway → service → Networking → Custom
...
- `schildlabel.com` is the **first/default** sending identity (`app/sending_domains.py`).
- It's in `REDIRECT_HOSTS` so Option 3 works out of the box.
- Reply-to allowlist accepts schildlabel.com.
- **You** still need: the Resend DNS + verify (Part A) and the redirect (Part B), and to add
  schildlabel.com to `SEND_VERIFIED_DOMAINS` once green.
